...
def main():
    ...
    parser.add_argument('--generate-sn', action='store_true', help='Flag to specify dataset generation from sensor network.')
    ...
    elif args.generate_sn:
        generate_sn(params['generate_sn'])
    elif args.test:
        if args.slsqp:
...
def generate(params):
    ...
    graph = load_grosmnxaph(graph_name=graph_name)
...
def generate_sn(params):
    ...
    #Read from .sn file to make nx graph

    with open(params['sn_name'], 'r') as file:
    # Read maximum x and y values for length and width (not used in this code snippet)
        max_x, max_y, transmission_range = map(float, file.readline().split())

        # Read capacity and packet count (not used in this code snippet)
        capacity, packet_count = map(int, file.readline().split())
        ...
        for _ in range(num_nodes):
            ...
            if node_type == 'd':
                packet_value = int(line[3])
                graph.add_node(node_type, x=x, y=y, packet_value=packet_value)
            else:
                graph.add_node(node_type, x=x, y=y)

        # Compute distances between nodes and add edges
        for u in graph.nodes():
            for v in graph.nodes():
                if u != v:
                    distance = ((graph.nodes[u]['x'] - graph.nodes[v]['x'])**2 + (graph.nodes[u]['y'] - graph.nodes[v]['y'])**2) ** 0.5
    ...
    G = graph.copy()
    for source in sources:
        G.add_node(source, demand=-packet_count)
    for sink in sinks:
        G.add_node(sink, demand=capacity)
    file_path = os.path.join(dataset_folder, 'graph')
    plot_road_flow_graph(graph=G, field='zero', graph_name=graph_name, file_path=file_path)
    ...
    for file_path, num_samples in zip(file_paths, samples):
        ...
        for i in range(num_samples):
            ...
            if file_path == 'train':
                ...
                sink_index.add_item(i, sink_demands)
            #elif file_path=='test':
                    #for now there is only one test sample, where the sources are the data nodes and the sinks are the storage nodes
            else:
                _, source_dist = source_index.get_nns_by_vector(source_demands, n=1, include_distances=True)
                _, sink_dist = source_index.get_nns_by_vector(sink_demands, n=1, include_distances=True)
